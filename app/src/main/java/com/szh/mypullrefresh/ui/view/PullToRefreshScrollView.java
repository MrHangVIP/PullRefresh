...
public class PullToRefreshScrollView extends PullToRefreshBase<ScrollView> {
	...
	private final OnRefreshListener<ScrollView> defualtRefreshImpl=new OnRefreshListener<ScrollView>() {
		@Override
			...
			new Thread(new Runnable() {
				...
				public void run() {
					refreshView.getRefreshableView().postDelayed(new Runnable() {
						@Override
						public void run() {
							Toast.makeText(refreshView.getRefreshableView().getContext()
									,"刷新完成",Toast.LENGTH_SHORT).show();
							onRefreshComplete();
						}
					},2000);
	...
	public PullToRefreshScrollView(Context context) {
		super(context);
		setOnRefreshListener(defualtRefreshImpl);
	}
